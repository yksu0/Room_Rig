...
class _ErgonomicsBenchPanelState extends State<ErgonomicsBenchPanel>
  ...
  ErgonomicsSimSnapshot? _baseSim;
  ErgonomicsSimSnapshot? _optSim;
  ErgonomicsSimSnapshot? _myRigSim;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AppState>().loadSimulatedPrototypeBaseline(mode: 'ergonomics');
      _rebuild();
    });
  }
  ...
  void _rebuild() {
    ...
    _baseline = ErgonomicsPrototypeLayouts.baseline(source);
    _improved = ErgonomicsPrototypeLayouts.optimized(source);
    _baseSim = ErgonomicsSimulator.build(furniture: _baseline, optimized: false);
    _optSim = ErgonomicsSimulator.build(furniture: _improved, optimized: true);
  ...
  void _rebuildMyRigSim() {
    ...
    _myRigSim = ErgonomicsSimulator.build(furniture: items, optimized: false);
  }
    ...
    return Column(
      ...
      children: [
        ...
        if (_step == _ErgoStep.layout) ...[
          ...
          const SizedBox(height: 16),
          _primaryButton(
            _layoutVariant == _Variant.myRig ? 'RUN MY RIG ERGONOMICS' : 'RUN ERGONOMICS BENCH',
            RoomSvg.scan,
          ...
          Text(
            _layoutVariant == _Variant.myRig
                ? 'Starts from Bench ergonomics layout (fan included), pushes into Rig — edit there, then re-run.'
                : 'Compare jammed vs clear walk routes (bed→PC, entry→desk), then score comfort.',
        ...
        if (_step == _ErgoStep.results && !_running) ...[
          ...
          Row(
            children: [
                      ...
                      SnackBar(
                        content: Text(
                          _simVariant == _Variant.myRig
                              ...
                              : 'Improved ergonomics layout applied to Rig',
                        ),
              ...
              _iconButton(() {
                state.loadSimulatedPrototypeBaseline(mode: 'ergonomics');
                _rebuild();
              }),
            ],
          ),
  ...
  Widget _layoutSection(AppState state) {
    final notes = switch (_layoutVariant) {
      _Variant.improved => ErgonomicsPrototypeLayouts.optimizedNotes,
      _Variant.myRig => const [
          'Seeded from Bench Current — includes the stand fan',
          'Pushed into Rig so you can drag furniture there',
        ],
      _Variant.current => ErgonomicsPrototypeLayouts.baselineNotes,
    };
        ...
        GlassCard(
          child: Column(
            ...
            children: [
              Text(
                switch (_layoutVariant) {
                  _Variant.current => 'Jammed work zone — chair blocked, gear out of reach',
                  _Variant.improved => 'Clear pull-back aisle + reach envelope at the desk',
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        ...
                        size: 14,
                        color: _layoutVariant == _Variant.improved
                            ? AppColors.green
                            : (_layoutVariant == _Variant.myRig ? AppColors.cyan : AppColors.amber),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
        ...
        Container(
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, child) => _orbit(
              enabled: _simViz == ErgonomicsVizMode.orbit3D,
              child: CustomPaint(
        ...
        children: [
          Row(
            children: [
              ...
              const Expanded(
                child: Text('Ergonomics Comfort Pass', style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w700, fontSize: 16)),
              ),
              Container(
          ...
          Row(
            ...
            children: [
              ScoreRing(score: base.comfortScore, size: 78, color: AppColors.amber, label: 'Before'),
              ScoreRing(score: opt.comfortScore, size: 78, color: AppColors.ergonomicsColor, label: 'After'),
              if (mine != null)
                ScoreRing(score: mine.comfortScore, size: 78, color: AppColors.cyan, label: 'My Rig')
              else
          ...
          BenchmarkValidationCard(
            validation: state.validateActiveLayout(mode: 'ergonomics'),
          ),
        ],
      ),
    );
